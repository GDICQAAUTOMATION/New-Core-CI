...
describe('GDIC CI Motor test', () => {
  ...
  // Select Vehicle Name as "Microbus"
  cy.log('Attempting to select Microbus in Vehicle Name');
  ...
  cy.get('.ng-dropdown-panel .ng-option').each(($el, index, $list) => {
    ...
    if (optionText === 'Microbus') {
        cy.wrap($el).scrollIntoView().click({ force: true });
        cy.log('Microbus found and clicked!');
    }
  });
  
  // Verify the Motor Cycle selection
  cy.get('ng-select[formcontrolname="mtrVehicleName"]').should('contain', 'Microbus', { timeout: 10000 });
  ...
  cy.get('ng-select[formcontrolname="mtrCcTon"]')
    .should('contain', 'SEAT');
  //SEAT value
    cy.get(':nth-child(12) > .input-group > .form-control').type('18')
  ...
  cy.get('.flex.mt-10 > :nth-child(2) > .select-group > .ng-select > .ng-select-container > .ng-value-container > .ng-input > input').type('{enter}')
   ...
   //submission 
    ...
    cy.get('.mr-10 > .ng-select > .ng-select-container > .ng-value-container > .ng-input > input').type('2001{enter}')
     ...
     cy.get('.form-footer > :nth-child(2) > .btn').click()
     cy.screenshot('Preview_Microbus')
  ...
  cy.screenshot('Riot strike added preview_microbus')
  cy.wait(1000)
  ...
  cy.screenshot('submitted_microbus')
